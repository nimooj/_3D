// GDialogBar.cpp : implementation file
//

#include "stdafx.h"
#include "../Gaia.h"
#include "afxpriv.h"    // for CDockContext
#include "GDialogBar.h"
...
GDialogBar::GDialogBar()
{
	...
	m_clrBtnHilight = RGB(0, 0, 0),
	m_clrBtnShadow = RGB(BC[0], BC[1], BC[2]);
	m_brushBkgd.CreateSolidBrush(RGB(BC[0], BC[1], BC[2]));

	
	//m_brushBkgd.CreateSolidBrush(RGB(68, 68, 68));
...
BEGIN_MESSAGE_MAP(GDialogBar, CControlBar)
	//{{AFX_MSG_MAP(GDialogBar)
	ON_WM_LBUTTONUP()
	ON_WM_MBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_WM_SETCURSOR()
...
/////////////////////////////////////////////////////////////////////////////
// GDialogBar message handlers

void GDialogBar::OnUpdateCmdUI(CFrameWnd *pTarget, int bDisableIfNoHndler)
{
	UpdateDialogControls(pTarget, bDisableIfNoHndler);
}

BOOL GDialogBar::Create(CWnd* pParentWnd, CDialog *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
...
CSize GDialogBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	CRect rc;
...
CSize GDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void GDialogBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
...
BOOL GDialogBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
// Mouse Handling
//
void GDialogBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if (!m_bTracking)
...
void GDialogBar::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void GDialogBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
void GDialogBar::OnNcPaint() 
{
	EraseNonClient();

	CWindowDC dc(this);
	dc.Draw3dRect(m_rectBorder, GetSysColor(COLOR_BTNHIGHLIGHT), GetSysColor(COLOR_BTNSHADOW));
	//dc.Draw3dRect(m_rectBorder, GetSysColor(COLOR_APPWORKSPACE), GetSysColor(COLOR_BTNSHADOW));
...
void GDialogBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
...
HITTEST_RET GDialogBar::OnNcHitTest(CPoint point) 
{
	if (IsFloating())
...
void GDialogBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void GDialogBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void GDialogBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void GDialogBar::StopTracking(BOOL bAccept)
{
	OnInvertTracker(m_rectTracker);
...
void GDialogBar::OnInvertTracker(const CRect& rect)
{
	ASSERT_VALID(this);
...
BOOL GDialogBar::IsHorz() const
{
	return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
		m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

CPoint& GDialogBar::ClientToWnd(CPoint& point)
{
...
void GDialogBar::DrawGripper(CDC & dc)
{
	// Floating 타입이면 gripper를 그리지 않는다.
...
void GDialogBar::OnNcLButtonDblClk(UINT nHitTest, CPoint point) 
{
